...
output "app_subnets" {
  value = toset(values({for i,j in lookup(module.vpc["main"],"app_subnets",null): i => j.id})[0])
}
...
#   count      = values({for i, j in lookup(module.vpc["main"], "public_subnets", null) : i => j.id})[0]
